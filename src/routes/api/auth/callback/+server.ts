...
//here is were the spotify api will go after the user has logged in
export const GET: RequestHandler = async ({ url, cookies }) => {
    const code = url.searchParams.get('code') || null;
    const state = url.searchParams.get('state') || null;

    //check if the state is the same as the state we have saved in the cookies
    //allso get challenge verifier from the cookies to verify the code sent from spotify
    ...
    //redirect to home, 303 temporary redirect
    throw redirect(303, '/');
