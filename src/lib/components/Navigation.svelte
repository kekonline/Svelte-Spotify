<script lang="ts">
	...
	// In despktop mode, the menu is always open
	let isMobileMenuOpen = false;
	//In mobile mode, the menu will be open and closed on click with this variable
...
<div class="nav-content" class:desktop class:mobile={!desktop}>
	<!-- This class overlays will be added only when the menu is open with the transition -->
	{#if !desktop && isMobileMenuOpen}
		<div
			class="overlay"
			on:click={closeMenu}
				...
				{#each menuItems as item, index}
					...
					<li class:active={item.path === $page.url.pathname}>
						{#if menuItems.length === index + 1}
							<!-- Focusable element will be used for the focus trap -->
